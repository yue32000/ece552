...
static counter_t total_stallq1;
static counter_t total_stallq2;
...
sim_reg_stats(struct stat_sdb_t *sdb)
{
  ...
  stat_reg_formula(sdb, "CPI_from_RAW_hazard_q1",
		   ...
		   "(sim_num_insn+total_stallq1)/sim_num_insn" /* ECE552 - MUST ADD YOUR FORMULA */, NULL);

  stat_reg_formula(sdb, "CPI_from_RAW_hazard_q2",
		   "CPI from RAW hazard (q2)",
		   "(sim_num_insn+total_stallq2)/sim_num_insn" /* ECE552 - MUST ADD YOUR FORMULA */, NULL);

  stat_reg_counter(sdb, "total_stallq1",
		   "total stall cycles caused by RAW hazards (q1)",
		   &total_stallq1, total_stallq1, NULL);
  stat_reg_counter(sdb, "total_stallq2",
		   "total stall cycles caused by RAW hazards (q2)",
		   &total_stallq2, total_stallq2, NULL);
...
sim_main(void)
{
  ...
  /* ECE552 Assignment1 - BEGIN CODE*/
  int r_out[2], r_in[3];
  /* ECE552 Assignment1 - END CODE*/
...
/* ECE552 Assignment-1 - BEGIN CODE*/
counter_t stall;
counter_t biggest_stall=0;
counter_t stallq2;
counter_t biggest_stallq2=0;
int i;
for (i = 0; i < 3; i++) 
{
...
for (int i=0;i<MD_TOTAL_REGS;i++)
	{
		reg_ready[i]-=biggest_stall;
	}
total_stallq1+=biggest_stall;
...
for (i = 0; i < 3; i++)
{
	...
	{
		stall = reg_readyq2 [r_in[i]] - sim_num_insn;	
		if (stall > biggest_stallq2)
		{
		
			biggest_stallq2 = stall;
		}
	}
}
if (biggest_stallq2 > 0)
{
	sim_num_RAW_hazard_q2++;
	for (int i = 0; i < MD_TOTAL_REGS; i++)
	{
		reg_readyq2[i]-=biggest_stallq2;
	}
	total_stallq2+=biggest_stallq2;
}
