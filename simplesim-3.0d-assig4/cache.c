...
/*ECE552 Assignment 4 - BEGIN CODE*/
if (prefetch_type>2){
	cp->rpt=(struct rpt_t*)malloc(cp->prefetch_type*sizeof(struct rpt_t));
    ...
    if (prefetch_type==2){
        cp->orpt=(struct rpt_t*)malloc(oRPT_size*sizeof(struct rpt_t));
        for(int i =0; i<oRPT_size; i++){
            cp->orpt[i].tag=0;
        }
    }
...
/*ECE552 Assignment 4 - BEGIN CODE*/
	int fetch_addr=(addr+cp->bsize)-((addr+cp->bsize)%cp->bsize);
	if(cache_probe(cp,fetch_addr)==0)
...
/*ECE552 Assignment 4 - BEGIN CODE*/
    md_addr_t prev_index=(prev_addr>>2)&8191;
    md_addr_t prev_tag=(prev_addr>>13)>>2;
    md_addr_t M_index=(addr>>2)&(8191);
    md_addr_t tag1=(addr>>13)>>2;
    md_addr_t fetch_addr;
    int stride_result=open_stride(cp,addr);
    if(stride_result==0){
		if(tag1!=M_tag[M_index]){
		    /*count1++;
		    if(count1%10000==0)
		        printf("count1 %d\n",count1);*/
		    fetch_addr=(addr+cp->bsize)-((addr+cp->bsize)%cp->bsize);
		    //if(cache_probe(cp,fetch_addr)==0)
		        //cache_access(cp,Read,fetch_addr,NULL,cp->bsize,0,NULL,NULL,1);
		    ...
		    fetch_addr=P1[M_index]-(P1[M_index]%cp->bsize);
		        if(cache_probe(cp,fetch_addr)==0){
		            /*count2++;
		            if(count2%1000==0)
		                printf("count2 %d\n",count2);*/
    ...
    if(miss)
		{
			if(M_tag[prev_index]==prev_tag){
				if(P1[prev_index]!=addr){
...
int open_stride(struct cache_t *cp, md_addr_t addr){
    //int RPT_size=cp->prefetch_type;
    
    md_addr_t PC_addr=get_PC();
    int index_mask=oRPT_size-1;
    md_addr_t RPT_index= (PC_addr>>3)&(oRPT_size-1);////////
    md_addr_t RPT_tag=(PC_addr>>3)-RPT_index;//////////
    //rpt_t * rpt=cp->rpt;
    if(RPT_tag==cp->orpt[RPT_index].tag){
        int stride= addr-(cp->orpt[RPT_index].prev_add);
        int fetch_addr=(addr+stride)-((addr+stride)%cp->bsize);
        cp->orpt[RPT_index].prev_add=addr;
        if(stride==cp->orpt[RPT_index].stride){
            switch (cp->orpt[RPT_index].state) {
                case Init:
                    cp->orpt[RPT_index].state=Steady;
                    if(cache_probe(cp,fetch_addr)==0)
                        cache_access(cp,Read,fetch_addr,NULL,cp->bsize,0,NULL,NULL,1);
                    //return true;
                    break;
                ...
                case Steady:
                    ...
                    break;
                case Nopred:
                    cp->orpt[RPT_index].state=Transient;
                    //if(cache_probe(cp,fetch_addr)==0)
                        //cache_access(cp,Read,fetch_addr,NULL,cp->bsize,0,NULL,NULL,1);
                   // return true;
                    return 0;
            ...
            switch (cp->orpt[RPT_index].state) {
                case Init:
                    cp->orpt[RPT_index].state=Transient;
                    cp->orpt[RPT_index].stride=stride;
                    //if(cache_probe(cp,fetch_addr)==0)
                        //cache_access(cp,Read,fetch_addr,NULL,cp->bsize,0,NULL,NULL,1);
                    return 0;
                    break;
                case Transient:
                    cp->orpt[RPT_index].state=Nopred;
                    cp->orpt[RPT_index].stride=stride;
                    //No cache_access
                    return 0;
                    break;
                case Steady:
                    cp->orpt[RPT_index].state=Init;
                    //if(cache_probe(cp,fetch_addr)==0)
                      //  cache_access(cp,Read,fetch_addr,NULL,cp->bsize,0,NULL,NULL,1);
                    return 0;
                    break;
    ...
    //the tag does not match
        ...
        cp->orpt[RPT_index].prev_add=addr;
        cp->orpt[RPT_index].tag=RPT_tag;
        cp->orpt[RPT_index].stride=0;
        cp->orpt[RPT_index].state=Init;
        return 0;
...
/*ECE552 Assignment 4 - BEGIN CODE*/
	...
	//make sure it is  a stride prefetch
    int RPT_size=cp->prefetch_type;
	
    md_addr_t PC_addr=get_PC();
    int index_mask=RPT_size-1;
    md_addr_t RPT_index= (PC_addr>>3)&(RPT_size-1);////////
	md_addr_t RPT_tag=(PC_addr>>3)-RPT_index;//////////
	//rpt_t * rpt=cp->rpt;
    if(RPT_tag==cp->rpt[RPT_index].tag&&cp->rpt[RPT_index].tag!=0){
        int stride= addr-(cp->rpt[RPT_index].prev_add);
		int fetch_addr=(addr+stride)-((addr+stride)%cp->bsize);
		
        if(stride==cp->rpt[RPT_index].stride){
            switch (cp->rpt[RPT_index].state) {
                case Init:
            ...
            cp->rpt[RPT_index].prev_add=addr;
                }
        else{
            switch (cp->rpt[RPT_index].state) {
                case Init:
                    ...
                    break;
                case Transient:
                    cp->rpt[RPT_index].state=Nopred;
                    cp->rpt[RPT_index].stride=stride;
                    //No cache_access
                    break;
                case Steady:
                    cp->rpt[RPT_index].state=Init;
					fetch_addr=(addr+cp->rpt[RPT_index].stride)-((addr+cp->rpt[RPT_index].stride)%cp->bsize);
                    if(cache_probe(cp,fetch_addr)==0)
                    cache_access(cp,Read,fetch_addr,NULL,cp->bsize,0,NULL,NULL,1);
                    break;
                case Nopred:
